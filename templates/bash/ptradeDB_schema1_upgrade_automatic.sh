...
create_user_scriptName="buildSchema_ptrade1.sh"
create_user_script="$create_user_scriptPath"/"$create_user_scriptName"
tarFilter="DB"
db_config_qa_ptrade_exampleName="db_config_qa_ptrade_example.sql"
db_config_qa_ptrade_examplePath="$scriptFolderPath/$db_config_qa_ptrade_exampleName"
...
{
	echo "******************************************************************************************************"
	echo "This is a script for ptradeDB schema1 upgrade"
...
if [ "$pathGiven" != "" ]; then
	...
	### return 1 if the files are not identical (byte-to-byte comparison)
	if cmp -s "$storagePath/$getLastPackageName" "$untarWhereTo/$archiveName"; then
		echo "The identical package is already in the target folder, continuing without copy"
	else
...
if [ "$pathGiven" != "" ]; then
	...
	upgradePtradeSql="${installationFolderName1##*"$tarFilter"_}" # getting version number from folder name
	installationFolderName="$installationFolderName1"_"$userToDrop"
	installationPath="$untarWhereTo"/"$installationFolderName"
	db_config_qa_ptrade_Path="$installationPath/db_config_qa.sql"
else
...
if [ -f "$db_config_qa_ptrade_Path" ]; then
	cat "$db_config_qa_ptrade_examplePath" > "$db_config_qa_ptrade_Path"
	echo -e "\e[32m$db_config_qa_ptrade_Path was set for $userToDrop:\n\e[0m"
	cat "$db_config_qa_ptrade_Path"
	echo ""
else
	echo -e "\e[31mError: \""$db_config_qa_ptrade_Path"\" not found, exiting\e[0m"
	exit 1
fi
...
### ***** creating schema *****
echoColoured "Start upgrading $userToDrop..." yellow

### defining error message for the following block
errorMsg="Error while upgrading schema, installation not finished"

cd "$installationPath"
chmod 755 *.sh

### getting version number from folder name
upgradePtradeSql=upgrade_ptrade_"$upgradePtradeSql".sql

./upgrade_ptrade_db.sh "$installPtradeMode" "$upgradePtradeSql"
