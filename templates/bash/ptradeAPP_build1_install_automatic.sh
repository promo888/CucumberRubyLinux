...
{
	echo "******************************************************************************************************"
	echo "This is a script for ptradeAPP installation for user ptrade1"
...
echoColoured "\nsetting PTS_AUTOMATION_INSTALLATION_PATH" yellow
export PTS_AUTOMATION_INSTALLATION_PATH="$installationPath"
echo "PTS_AUTOMATION_INSTALLATION_PATH: $PTS_AUTOMATION_INSTALLATION_PATH"
...
if {[info exists env(PTS_AUTOMATION_LOGS_HOME)]} {
	...
	puts \"Error while reading env variable PTS_AUTOMATION_LOGS_HOME\"
	exit 1
}
...
if {[info exists env(PTS_AUTOMATION_JAVA_HOME)]} {
	...
	puts \"Error while reading env variable PTS_AUTOMATION_JAVA_HOME\"
	exit 1
}

if {[info exists env(PTS_AUTOMATION_INSTALLATION_PATH)]} {
    set automation_installation_path $::env(PTS_AUTOMATION_INSTALLATION_PATH)
	puts \"Found PTS_AUTOMATION_INSTALLATION_PATH\"
} else {
	puts \"Error while reading env variable PTS_AUTOMATION_INSTALLATION_PATH\"
	exit 1
}
...
spawn bash
sleep 1
send \"cd \$automation_installation_path && ./pts_installer.sh -u\r\"
sleep 3
