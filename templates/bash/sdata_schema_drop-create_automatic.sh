...
db_config_sdata_exampleName="db_config_sdata_example.sql"
...
{
	echo "******************************************************************************************************"
	echo "This is a script for sdataDB schema installation"
...
echoColoured "Creating user $userToDrop..." yellow
...
begin
        execute immediate 'CREATE BIGFILE TABLESPACE SDATADATA LOGGING DATAFILE SIZE 500M AUTOEXTEND ON NEXT  50M MAXSIZE UNLIMITED EXTENT MANAGEMENT LOCAL SEGMENT SPACE MANAGEMENT  AUTO';
exception
...
begin
        execute immediate 'CREATE BIGFILE TABLESPACE SDATAINDX LOGGING DATAFILE SIZE 500M AUTOEXTEND ON NEXT  50M MAXSIZE UNLIMITED EXTENT MANAGEMENT LOCAL SEGMENT SPACE MANAGEMENT  AUTO';
exception
        when others then null;
end;
/
CREATE USER SDATA IDENTIFIED BY sdata
        DEFAULT TABLESPACE SDATADATA
        TEMPORARY TABLESPACE TEMP
        QUOTA UNLIMITED ON SDATADATA
        QUOTA UNLIMITED ON SDATAINDX;

GRANT CONNECT, RESOURCE, CREATE VIEW, CREATE JOB, CREATE MATERIALIZED VIEW, UNLIMITED TABLESPACE TO SDATA;
GRANT CREATE SYNONYM TO SDATA;
GRANT READ, WRITE ON DIRECTORY DATA_PUMP_DIR TO SDATA;
grant execute on DBMS_SQL to SDATA;
grant execute on DBMS_LOB to SDATA;
grant execute on dbms_application_info  to SDATA;
grant execute on dbms_xplan to SDATA;
grant select any dictionary to SDATA;
grant execute on dbms_workload_repository to SDATA;
grant create table to SDATA;
...
echoColoured "Creating $userToDrop user completed" green
echoColoured ""
}
...
### Let's go
echoColoured "\n$(date +%Y-%m-%d_%H:%M:%S,%3N) -- Starting..." yellow
...
echoColoured "\n$(date +%Y-%m-%d_%H:%M:%S,%3N) -- Finished" green