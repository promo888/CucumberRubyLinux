...
{
	echo "******************************************************************************************************"
	echo "This is a script for ptradeDB schema1 installation"
...
begin
    ...
    if upper('^dropUser') not in ('PTRADE1') then
...
if [[ -x "$create_user_script" ]]; then
	...
	echoColoured "Error: script for creating user \""$create_user_script"\" does not exist or not executable, exiting" red
	exit 1
fi
